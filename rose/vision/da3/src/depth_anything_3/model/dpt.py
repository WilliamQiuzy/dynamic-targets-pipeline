...
class ResidualConvUnit(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # type: ignore[override]
        ...
        # ROSE torch-2.8 patch: FloatFunctional.add == torch.add (+ Identity) for fp
        # models, but its inner module-call breaks under FX tracing when SAM3's
        # concurrent dynamo-compile leaks FX state into the DA3 thread
        # ("module is not installed as a submodule"). Plain add is numerically identical.
        return out + x
...
class FeatureFusionBlock(nn.Module):
    ...
    def forward(self, *xs: torch.Tensor, size: Tuple[int, int] = None) -> torch.Tensor:  # type: ignore[override]
        ...
        if self.has_residual and len(xs) > 1 and self.resConfUnit1 is not None:
            y = y + self.resConfUnit1(xs[1])  # ROSE torch-2.8 patch (see above)
