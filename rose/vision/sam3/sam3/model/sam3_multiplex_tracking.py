...
class Sam3MultiplexTracking(Sam3MultiplexBase):
    ...
    def add_prompt(
        ...
        _skip_reset: ROSE pipeline patch — when True, do NOT call reset_state.
        Used to add bbox prompts at multiple frames within a single session
        (multi-anchor batched init).  Default False preserves the original
        behaviour.
        """
        logger.info("Running add_prompt on frame %d (skip_reset=%s)", frame_idx, _skip_reset)
        ...
        # since it's a semantic prompt, we start over — unless caller wants
        # to preserve prior prompts (multi-anchor batched init).
        if not _skip_reset:
            self.reset_state(inference_state)
...
class Sam3MultiplexTrackingWithInteractivity(Sam3MultiplexTracking):
    ...
    def _prepare_backbone_feats(self, inference_state, frame_idx, reverse):
        feature_cache = inference_state["feature_cache"]
        # ROSE: if this frame's backbone features are still cached (retained from the
        # B-3/B-4 grounding pass when skip_b6_redetect is on), reuse them — skip the
        # full run_backbone_and_detection, which would re-run the FA detection (~0.05s)
        # only to recompute backbone features we already have. ~1.5s/video saved.
        if getattr(self, "_rose_skip_b6_redetect", False) and frame_idx in feature_cache:
            return
        input_batch = inference_state["input_batch"]
        num_frames = inference_state["num_frames"]
        geometric_prompt = (
