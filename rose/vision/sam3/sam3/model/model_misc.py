...
def multi_head_attention_forward(
    ...
    if attn_type == AttentionType.Vanilla:
        # ROSE 2026-05-13: ~17% of masked SDPA fallback calls pass an
        # all-zero additive mask (mathematically a no-op) — see
        # scripts/analyze_attn_masks.py. Detect those and route to FA3.
        # Skip the check for masks > 128 elements to avoid GPU-CPU sync
        # cost on the large additive bias masks (shape e.g. (1,8,201,5184))
        # which are NEVER no-ops; the all-zero masks measured were ≤72 elts.
        effective_mask_is_noop = False
        if (
            attn_mask is not None
            and use_fa3
            and not is_causal
            and attn_mask.numel() <= 128
        ):
            if attn_mask.dtype == torch.bool:
                effective_mask_is_noop = bool(attn_mask.all().item())
            else:
                effective_mask_is_noop = not bool((attn_mask != 0).any().item())
        if (attn_mask is None or effective_mask_is_noop) and not is_causal and use_fa3:
        ...
        if not self._qkv_same_embed_dim:
            ...
            if self.use_act_checkpoint:
                attn_output, attn_output_weights = torch.utils.checkpoint.checkpoint(
                    ...
                    attn_sparsity=self.sparsity,
                    attn_bias=attn_bias,
                    use_fa3=self.use_fa3,  # ROSE 2026-05-13: upstream bug — qkv-same-embed branch dropped use_fa3
                )
            else:
                attn_output, attn_output_weights = multi_head_attention_forward(
                    query,
