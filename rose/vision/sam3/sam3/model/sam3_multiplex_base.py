...
class Sam3MultiplexBase(Sam3VideoBase):
    ...
    def _det_track_one_frame_impl(
        ...
        with torch.profiler.record_function("GPU sync and filter"):
            ...
            # CUDA argsort doesn't support bool dtype — cast to int8
            pos_pred_mask_idx = pos_pred_mask.to(torch.int8).argsort(descending=True)
    ...
    def run_backbone_and_detection(
        ...
        # ROSE: when skip_b6_redetect is on, KEEP all per-frame features so B-6's
        # partial propagation can reuse them instead of re-running the FA detection
        # (saves ~1.5s/video; H200 has ample memory for ~17 strided frames).
        if not getattr(self, "_rose_skip_b6_redetect", False):
