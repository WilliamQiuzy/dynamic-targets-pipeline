...
def build_sam3_multiplex_video_model(
    ...
    tri_neck = _create_multiplex_tri_backbone(
        # ROSE torch-2.8 patch: was "max-autotune". On variable real-video shapes
        # (frame-count → batch dim, object counts) max-autotune RE-BENCHMARKS kernels
        # per new shape → an autotune storm (30k+ kernels) that never settles across
        # 8 fresh videos. "default" still fuses + cuts launches (the latency-bound win)
        # but compiles each shape cheaply (no kernel benchmarking). max-autotune only
        # pays off when ALL shapes are pre-warmed once into the persistent disk cache.
        compile_mode="default" if compile else None
    )
...
def build_sam3_multiplex_video_predictor(
    ...
    tracker_model = build_sam3_multiplex_video_model(
        ...
        use_fa3=use_fa3,
        use_rope_real=use_rope_real,
        compile=compile,  # propagate compile flag (was hardcoded False!)
