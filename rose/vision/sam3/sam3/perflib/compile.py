...
def compile_wrapper(
    ...
    cudagraphs disabled: tested with cudagraphs ON (v52) — Qwen-VL allocator
    activity between SAM3 sessions corrupts cached graphs.  Symptoms:
    - per-video tracker output regresses (track count drops, obs goes from
      32 → 4 for some videos)
    - per-video Hz drops from 5.79 (cudagraphs OFF) to 2.67 (cudagraphs ON)
    Even with cleanup `finally` block in `_run_gpu_phase`, the allocator
    history that cudagraphs cached doesn't match next-video state.
    Forcing "max-autotune-no-cudagraphs" is the stable choice — kernels
    are still autotuned but no graph capture/replay is attempted.
    """
    safe_mode = "max-autotune-no-cudagraphs" if mode == "max-autotune" else mode
    compiled_fn = torch.compile(fn, mode=safe_mode, fullgraph=fullgraph, dynamic=dynamic)
